...
class OrderBookAnalyzer:
    ...
    def _validate_orderbook(self, symbol: str, current_ob: dict) -> bool:
        ...
        if current_ob['bids'][0][0] >= current_ob['asks'][0][0]:
            if abs(current_ob['bids'][0][0] - current_ob['asks'][0][0]) < 1e-8:
                logger.debug(f"Minor spread anomaly for {symbol}: bid={current_ob['bids'][0][0]}, ask={current_ob['asks'][0][0]}. Ignored.")
                return True
            logger.warning(
    ...
    async def _create_layering_alert(self, symbol: str, side: str, signals: Dict, price: float, current_ob: Dict):
        ...
        await self.db_manager.insert_alert(
            ...
            trade_history=list(self.trade_processor.trade_history.get(symbol, deque())) if self.trade_processor else None
        )
