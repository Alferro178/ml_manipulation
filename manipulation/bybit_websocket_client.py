...
from manipulation.db_manager import DBManager
...
async def handle_message(symbol: str, message: Dict[str, Any]):
    ...
    try:
        ...
        if "orderbook.500" in topic:
            ...
            logger.debug(f"Received publicTrade data for {symbol}: {len(data)} trades")
...
async def _handle_orderbook_message(symbol: str, message: Dict[str, Any], data: Dict[str, Any]):
    ...
    try:
        ...
        if _orderbook_analyzer:
            try:
                sorted_bids = sorted([[p, s] for p, s in orderbooks_by_symbol[symbol]['bids'].items()],
                                     key=lambda x: x[0], reverse=True)
                sorted_asks = sorted([[p, s] for p, s in orderbooks_by_symbol[symbol]['asks'].items()],
                                     key=lambda x: x[0])

                await _orderbook_analyzer.update_orderbook(symbol, sorted_bids, sorted_asks)
...
def close_port(port: int):
    """Проверка и освобождение порта, если он занят"""
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            try:
                ...
                if e.errno == 10048:  # Порт занят
                    logger.warning(f"Port {port} is in use, attempting to close existing connections")
                else:
                    logger.error(f"Error checking port {port}: {e}")
                    return False

        # Ищем процессы, использующие порт, исключая текущий процесс
        for conn in psutil.net_connections():
            if conn.laddr.port == port and conn.status == psutil.CONN_LISTEN and conn.pid != current_pid:
                pid = conn.pid
                if pid:
                    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            try:
                s.bind(('127.0.0.1', port))
                logger.debug(f"Port {port} is now free after cleanup")
                return True
            except OSError as e:
                logger.error(f"Port {port} still in use after cleanup: {e}")
                return False

    except Exception as e:
        logger.error(f"Error in close_port for {port}: {e}")
        return False
...
async def sync_subscriptions():
    """Периодическая синхронизация подписок с watchlist"""
    while True:
        try:
            ...
            missing_symbols = [s for s in symbols if s not in subscribed_symbols]
            if missing_symbols:
                logger.info(f"Found {len(missing_symbols)} missing symbols: {missing_symbols}")
                batch_size = 30
                batches = [missing_symbols[i:i + batch_size] for i in range(0, len(missing_symbols), batch_size)]
                for batch in batches:
                    asyncio.create_task(connect_to_bybit_chunk(batch))
            else:
                logger.debug("All watchlist symbols are subscribed")
...
async def connect_to_bybit_all(testnet: bool = False):
    ...
    tasks = [asyncio.create_task(connect_to_bybit_chunk(batch, testnet)) for batch in batches]
    tasks.append(asyncio.create_task(sync_subscriptions()))
    await asyncio.gather(*tasks, return_exceptions=True)
...
async def start_bybit_data_stream(direct_run_symbols: Optional[List[str]] = None):
    ...
    local_ws_port = int(os.getenv("LOCAL_WS_PORT", 8767))

    if not close_port(local_ws_port):
        logger.critical(f"Cannot start WebSocket server: port {local_ws_port} is still in use")
        raise RuntimeError(f"Port {local_ws_port} is in use")

    try:
        bybit_task = asyncio.create_task(connect_to_bybit_all())
        logger.info(f"Starting local WS server on ws://localhost:{local_ws_port}")
        async with websockets.serve(ws_handler, "localhost", local_ws_port) as server:
            await asyncio.gather(bybit_task, return_exceptions=True)
...
if __name__ == "__main__":
    ...
    orderbook_analyzer_instance = OrderBookAnalyzer(
        ...
        iceberg_trade_window_sec=int(config.get("ICEBERG_WINDOW_SEC", 60)),
    ...
    ml_data_collector_instance = MLDataCollector(
        ...
        orderbook_analyzer=orderbook_analyzer_instance
    )
