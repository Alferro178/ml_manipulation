...
import json
import time
import numpy as np  # Добавлено для обработки numpy-типов
...
class DBManager:
    ...
    async def insert_alert(self, symbol: str, alert_type: str, price: float, message: str,
                           ...
                           trade_history: Optional[List[Dict]] = None) -> Optional[int]:
        ...
        async with self.conn_pool.acquire() as conn:
            ...
            try:
                ...
                alert_id = await conn.fetchval(
                    ...
                    json.dumps(order_book_snapshot) if order_book_snapshot else None,
    ...
    async def _check_alert_grouping(self, conn, symbol: str, alert_type: str, current_timestamp_ms: int) -> Dict[str, Any]:
    ...
    async def _update_grouped_alert(self, conn, alert_id: int, new_timestamp_ms: int,
                                    ...
                                    ml_source_alert_type: Optional[str]):
        ...
        async with self.conn_pool.acquire() as conn:
            try:
                record_id = await conn.fetchval(
                    ...
                    """,
                    symbol,
                    json.dumps(features, default=convert_numpy_types),
                    target_price_change,
                    target_direction,
                    alert_id
                )
                ...
                logger.error(f"Error inserting ML training data: {e}")
                raise
