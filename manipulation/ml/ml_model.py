...
class MLModel:
    ...
    def _initialize_online_models(self):
        ...
        try:
            ...
            self.online_direction_model = SGDClassifier(
                ...
                random_state=42,
                warm_start=True,
                loss='log_loss'  # Исправлено: 'log' заменено на 'log_loss'
            )
    ...
    def _normalize_features(self, features: Dict[str, float]) -> Dict[str, float]:
        ...
        try:
            if not features:  # Добавлено: Проверка на пустые признаки
                logger.warning("Empty features provided for normalization")
                return {}
            ...
            for key, value in features.items():
                ...
                if 'price' in key.lower() or 'volume' in key.lower():
                    ...
                    normalized_features[key] = np.clip(value, -10, 10)
                elif 'imbalance' in key.lower():
                    # Для дисбалансов ограничиваем диапазон [-1, 1]
                    normalized_features[key] = np.clip(value, -1, 1)
                elif 'depth' in key.lower() or 'liquidity' in key.lower():
                    # Для глубины и ликвидности используем логарифмическое масштабирование
                    if value > 0:
                        normalized_features[key] = np.log1p(value)
                    else:
                        normalized_features[key] = 0
                    ...
                    if value >= 0:
                        normalized_features[key] = np.sqrt(value)
                    else:
                        normalized_features[key] = 0
                    ...
                    normalized_features[key] = np.clip(value, -100, 100)
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'models_trained': {
                ...
                'direction_model': self.direction_model is not None,
                'online_price_model': self.online_price_model is not None,
                'online_direction_model': self.online_direction_model is not None
            }
        }
    ...
    def get_model_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'online_models_trained': (self.online_price_model is not None and
                                      self.online_direction_model is not None),
