...
const MLDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Получаем предсказания для основных символов
      ...
      toast({
        ...
        description: "Не удалось получить данные с сервера. Проверьте подключение к backend.",
        variant: "destructive",
      });
    }
  };
                    ...
                    return (
                      <div key={symbol} className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                        <div className="flex items-center gap-3">
                          {prediction ? getPredictionIcon(prediction.predicted_direction) : <Activity className="h-4 w-4 text-gray-400" />}
                          <div>
                            <div className="font-medium">{symbol}</div>
                            <div className="text-xs text-muted-foreground">
                              {prediction ? formatTimestamp(prediction.timestamp) : 'Нет данных'}
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
